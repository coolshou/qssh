...
namespace QSsh
{
    namespace Internal
    {
        ...
        template <typename T>
        static T fromBigEndian(const QByteArray &ba)
        {
        ...
        void SshAgent::refreshKeysImpl()
        {
            ...
            const auto keysRequestIt = std::find_if(m_pendingRequests.constBegin(),
                                                    m_pendingRequests.constEnd(), [](const Request &r)
                                                    { return r.isKeysRequest(); });
        ...
        void SshAgent::handleIncomingData()
        {
            ...
            while (!hasError() && !m_incomingData.isEmpty())
            {
                if (m_incomingPacket.size == 0)
                {
                    ...
                    if (m_incomingData.length() < int(sizeof m_incomingPacket.size))
                        break;
                ...
                const int bytesToTake = qMin<quint32>(m_incomingPacket.size - m_incomingPacket.data.length(),
                                                      m_incomingData.length());
