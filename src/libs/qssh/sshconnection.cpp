...
namespace QSsh
{
    ...
    SshConnectionParameters::SshConnectionParameters() : timeout(0), authenticationType(AuthenticationTypePublicKey),
    ...
    {
        return p1.url == p2.url && p1.authenticationType == p2.authenticationType && p1.privateKeyFile == p2.privateKeyFile && p1.hostKeyCheckingMode == p2.hostKeyCheckingMode && p1.timeout == p2.timeout;
    }
    ...
    namespace Internal
    {
        ...
        void SshConnectionPrivate::setupPacketHandlers()
        {
            ...
            setupPacketHandler(SSH_MSG_KEXINIT, StateList() << SocketConnected << ConnectionEstablished, &This::handleKeyExchangeInitPacket);
            setupPacketHandler(SSH_MSG_KEXDH_REPLY, StateList() << SocketConnected << ConnectionEstablished, &This::handleKeyExchangeReplyPacket);

            setupPacketHandler(SSH_MSG_NEWKEYS, StateList() << SocketConnected << ConnectionEstablished, &This::handleNewKeysPacket);
            setupPacketHandler(SSH_MSG_SERVICE_ACCEPT,
                               StateList() << UserAuthServiceRequested,
                               &This::handleServiceAcceptPacket);
            if (m_connParams.authenticationType == SshConnectionParameters::AuthenticationTypePassword || m_connParams.authenticationType == SshConnectionParameters::AuthenticationTypeTryAllPasswordBasedMethods)
            {
            ...
            if (m_connParams.authenticationType == SshConnectionParameters::AuthenticationTypeKeyboardInteractive || m_connParams.authenticationType == SshConnectionParameters::AuthenticationTypeTryAllPasswordBasedMethods)
            {
            ...
            const StateList connectedList = StateList() << ConnectionEstablished;
            ...
            const StateList connectedOrClosedList = StateList() << SocketUnconnected << ConnectionEstablished;
            ...
            setupPacketHandler(SSH_MSG_DISCONNECT, StateList() << SocketConnected << WaitingForAgentKeys << UserAuthServiceRequested << UserAuthRequested << ConnectionEstablished, &This::handleDisconnect);
        ...
        void SshConnectionPrivate::handleIncomingData()
        {
            ...
            try
            {
                ...
                qCDebug(sshLog, "state = %d, remote data size = %d", int(m_state), int(m_incomingData.length()));
        ...
        void SshConnectionPrivate::handleServerId()
        {
            qCDebug(sshLog, "%s: incoming data size = %d, incoming data = '%s'",
                    Q_FUNC_INFO, int(m_incomingData.length()), m_incomingData.data());
        ...
        void SshConnectionPrivate::handleCurrentPacket()
        {
            ...
            QHash<SshPacketType, HandlerInStates>::ConstIterator it = m_packetHandlers.constFind(m_incomingPacket.type());
        ...
        {
            if (m_keyExchangeState != NoKeyExchange && m_keyExchangeState != KexInitSent)
            {
                throw SshServerException(SSH_DISCONNECT_PROTOCOL_ERROR,
        ...
        void SshConnectionPrivate::handleUserAuthInfoRequestPacket()
        {
            ...
            const SshUserAuthInfoRequestPacket requestPacket = m_incomingPacket.extractUserAuthInfoRequest();
                                                   ...
                                                   const QString &userErrorString)
        {
            ...
            {
            } // Nothing sensible to be done here.
            if (m_error != SshNoError)
