    ...
    namespace Internal
    {
        ...
        public:
            ...
            void establishConnection(const X11DisplayInfo &displayInfo)
            {
                ...
                const bool hasActualHostName = !displayInfo.hostName.isEmpty() && displayInfo.hostName != QLatin1String("unix") && !displayInfo.hostName.endsWith(QLatin1String("/unix")) && !hostNameIsPath;
            ...
            bool hasError() const
            {
                return (localSocket() && localSocket()->error() != QLocalSocket::UnknownSocketError) || (tcpSocket() && tcpSocket()->error() != QTcpSocket::UnknownSocketError);
            }

            bool isConnected() const
            {
                return (localSocket() && localSocket()->state() == QLocalSocket::ConnectedState) || (tcpSocket() && tcpSocket()->state() == QTcpSocket::ConnectedState);
            }
